...
if(lc($opts{t}) eq "pathway" || lc($opts{t}) eq "disease"){
	...
	print PWP_1 "#Term\tDatabase\tId\tSample number\tBackground number\tP-Value\tCorrected P-Value\tGenes\tHyperlink\n";	
...
#		}
	...
	print PWD_1 "#Term\tDatabase\tId\tSample number\tBackground number\tP-Value\tCorrected P-Value\tGenes\tHyperlink\n";
	for(my $i = 1; $i < @line; $i ++)
	{
		next if($line[$i] !~ /^\w+/);
		my @tmp = split /\t/, $line[$i];
...
###		}
	...
	close PWD_1;
	## add pathway class
	system "tabletools_add.pl -i /mnt/ilustre/users/bingxu.liu/workspace/RNA_Pipeline/RNA_database/KEGG/pathwayName_typeII_typeI -t $opts{i}.pathway.xls -n 1 -headt T -headi T > $opts{i}.pathway_addclass.xls";
	system "tabletools_add.pl -i /mnt/ilustre/users/bingxu.liu/workspace/RNA_Pipeline/RNA_database/KEGG/pathwayName_typeII_typeI -t $opts{i}.disease.xls -n 1 -headt T -headi T > $opts{i}.disease_addclass.xls";
}
...
if(type==\"PATHWAY\"){
	PATHWAY_type<-read.delim(\"/mnt/ilustre/users/bingxu.liu/workspace/RNA_Pipeline/RNA_database/KEGG/pathwayName_typeII_typeI\",sep=\"\\t\",header=T,check.names=F)
